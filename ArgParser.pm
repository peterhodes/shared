...
sub readargs {
    ...
    foreach $part (split('\s+(?=[a-z]+=|-)',$line)) { # Split on "-" or "=" that is preceeded by [a-z]+ that is itself preceeded by whitesp
ace.
        ...
        if ( ( $flag_dash ) and ( ! $flag_equal ) ) { # When we get something like -lrt we want to split that into seperate flags, of one c
haracter each.
            @argsplit = split(//,$arg);
...
sub getarg {
    ...
    local $value = $args{$arg}{VALUE};
    local $count = $args{$arg}{COUNT};
    return ($value,$count);
} # endsub getarg
################################################################################
